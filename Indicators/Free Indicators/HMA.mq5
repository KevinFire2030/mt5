...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   ArrayResize(diff_buffer, rates_total);
   ...
   for(int i = start; i < rates_total && !IsStopped(); i++)
     {
      ...
      diff_buffer[i] = 2 * wma1 - wma2;
     }
   
   // HMA 값 계산
   int hma_period = (int)MathSqrt(InpPeriod);
   for(int i = start; i < rates_total && !IsStopped(); i++)
     {
      HMABuffer[i] = iMAOnArray(diff_buffer, 0, hma_period, 0, MODE_LWMA, i);
     }
