...
int OnInit()
{
    ...
    // 지표 이름 설정
    IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("Tick Chart (%d)", InpTickInterval));
    
    // 플롯 설정 수정
    PlotIndexSetInteger(0, PLOT_DRAW_TYPE, DRAW_COLOR_CANDLES);
    PlotIndexSetInteger(0, PLOT_LINE_WIDTH, 1);
    PlotIndexSetInteger(0, PLOT_SHOW_DATA, true);
    PlotIndexSetInteger(0, PLOT_SHIFT, 0);  // 시프트 없음
...
void LoadHistory()
{
    ...
    if(received <= 0)
    {
        Print("틱 데이터 로드 실패! 에러 코드: ", GetLastError());
        return;
    }
